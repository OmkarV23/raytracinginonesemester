...
struct Light {
    // Point light fields (type == 0)
    Vec3 position     = make_vec3(0.0f, 0.0f, 0.0f);  // center for area lights too
    Vec3 color        = make_vec3(1.0f, 1.0f, 1.0f);
    int  intensity    = 1;

    // Area light extension (type == 1)
    int  type         = 0;                              // 0 = point, 1 = area
    Vec3 edge_u       = make_vec3(0.0f, 0.0f, 0.0f);  // half-width vector
    Vec3 edge_v       = make_vec3(0.0f, 0.0f, 0.0f);  // half-height vector
    Vec3 emission     = make_vec3(0.0f, 0.0f, 0.0f);  // emitted radiance Le
    Vec3 light_normal = make_vec3(0.0f, 1.0f, 0.0f);  // outward normal
    float area        = 1.0f;                           // total surface area
};
...
inline bool parse_scene(const JsonValue& root, Scene& scene, std::string* err) {
    ...
    // Helper lambda to parse one light JSON object into a Light struct.
    auto parse_one_light = [](const JsonValue& item) -> Light {
        ...
        if (json_get(item, "light_type", &ltype) &&
            ...
            // Accept either explicit edge_u / edge_v or normal + width + height
            ...
            if (json_get(item, "edge_u", &eu) && json_get(item, "edge_v", &ev)) {
                ...
                // Build edge vectors from normal + dimensions
                ...
                float upLen = sqrtf(dot(up, up));  // always 1 but keep it explicit
            ...
            // Derive normal and area from edge vectors
            Vec3 cr = cross(lc.edge_u, lc.edge_v);
            float crLen = sqrtf(dot(cr, cr));
            if (crLen > 1e-10f) {
                lc.light_normal = cr * (1.0f / crLen);
                lc.area = crLen * 4.0f;  // full rectangle = 4 * |edge_u x edge_v|
            }
        }
        return lc;
    };
