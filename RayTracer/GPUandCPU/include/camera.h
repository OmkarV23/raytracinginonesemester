...
class Camera {
  public:
    ...
    // cos_theta = forward · unit(p - center). Used by t=1 light tracing splat.
    HYBRID_FUNC bool world_to_pixel(const point3& p, float& px, float& py, float& cos_theta) const {
        ...
        // Ray center + t * unit(d) hits image plane at t = fl / cos_theta.
        // Equivalently center + (fl/dot_fd) * d.
        const vec3 hit_world = center + d * (fl_m / dot_fd);

        // pixel00_loc is the CENTER of pixel (0,0). Image-plane corner is half a pixel before it.
    ...
    // Image-plane area projected to z=1 (camera-coord depth 1). Used in PBRT-style
    // pinhole We = 1 / (A_z1 * cos⁴θ).
